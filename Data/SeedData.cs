...
public static class SeedData
{
    public static async Task EnsureMinimalFinanceSeedAsync(IServiceProvider services)
    {
        ...
        if (!await context.Departments.AnyAsync(x => x.CompanyId == company.Id))
        {
            var departments = new[]
            {
                new Department { TenantId = company.TenantId, CompanyId = company.Id, Code = "MKT", Name = "Marketing", CreatedAt = now },
                new Department { TenantId = company.TenantId, CompanyId = company.Id, Code = "FIN", Name = "Finance", CreatedAt = now },
                new Department { TenantId = company.TenantId, CompanyId = company.Id, Code = "OPS", Name = "Operations", CreatedAt = now }
            };
