...
public sealed class PaymentRequestService(ApplicationDbContext context) : IPaymentRequestService
{
    ...
    public async Task<PaymentRequestLookupsDto> GetLookupsAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            Departments = await context.Departments.AsNoTracking().OrderBy(x => x.Name).Select(x => new IntLookupItemDto { Id = x.Id, Name = x.Name }).ToListAsync(cancellationToken),
    ...
    private async Task ValidateReferencesAsync(int departmentId, Guid categoryId, Guid? vendorId, Guid? budgetId, CancellationToken cancellationToken)
    {
