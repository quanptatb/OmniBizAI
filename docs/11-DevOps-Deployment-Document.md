...
|------------|---------|-----|----------|-----------|
| **Local Dev** | Individual development | localhost:5000 / :5001 | Docker SQL Server | Docker Compose |
...
services:
  ...
  web:
    ...
    environment:
      ...
      AllowedOrigins: ${ALLOWED_ORIGINS:-http://localhost:5000}
    depends_on:
      sqlserver:
...
# JWT (external API clients only; MVC uses ASP.NET Identity cookie auth)
...
```bash
# Create migration
cd src/OmniBizAI.Infrastructure
...
|-----------|------|--------|
| ASP.NET Core MVC Web App | Serilog | Console + File (/var/log/omnibiz/) |
...
# JSON API: http://localhost:5000/api/v1
...
# Run ASP.NET Core MVC web app (hot reload)
cd src/OmniBizAI.Web
dotnet watch run
```
