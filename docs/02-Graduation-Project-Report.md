...
| Hình 5.2 | Giao diện Kanban workflow | Cần bổ sung ảnh màn hình |
| Hình 5.3 | Giao diện AI insight | Cần bổ sung ảnh màn hình |
...
| Vận hành | Tạo, xử lý, phê duyệt và theo dõi yêu cầu/công việc bằng Kanban |
...
- Module Kanban workflow để theo dõi công việc theo trạng thái.
...
Trong đề tài, workflow không chỉ dừng ở bước phê duyệt. Sau khi yêu cầu được duyệt, yêu cầu có thể được tách thành các công việc nhỏ để phòng ban thực hiện, phân công nhân sự, theo dõi hạn xử lý và cập nhật tiến độ.

### 2.5. Kanban trong quản lý workflow

Kanban là phương pháp trực quan hóa công việc theo các cột trạng thái. Thay vì chỉ xem danh sách yêu cầu, người quản lý có thể nhìn thấy công việc đang nằm ở đâu, việc nào bị vướng, việc nào quá hạn và ai đang phụ trách. Trong OmniBizAI, Kanban được dùng cho luồng công việc nội bộ sau khi yêu cầu vận hành được tạo/phê duyệt.

Các cột Kanban chính trong hệ thống:

| Cột | Ý nghĩa |
| --- | --- |
| Cần làm | Công việc đã được ghi nhận và chờ xử lý |
| Đang xử lý | Công việc đang được thực hiện |
| Đang vướng | Công việc bị chặn bởi thiếu thông tin, nguồn lực hoặc phê duyệt |
| Hoàn thành | Công việc đã xử lý xong |
| Đã hủy | Công việc không tiếp tục thực hiện |
...
| Trưởng bộ phận | Phân công, duyệt yêu cầu, theo dõi Kanban và KPI |
| Nhân viên | Tạo yêu cầu, cập nhật công việc trên Kanban |
...
flowchart LR
    ...
    Staff --> UC9["Cập nhật thẻ Kanban"]
    Manager["Trưởng bộ phận"] --> UC4["Duyệt và phân công"]
    Manager --> UC10["Quản lý Kanban workflow"]
...
| FR-11 | Quản lý công việc bằng Kanban theo trạng thái, phòng ban, người phụ trách và hạn xử lý | Must |
...
flowchart TB
    ...
    Work --> Kanban["Theo dõi trên Kanban"]
    Kanban --> Complete["Hoàn thành"]
...
erDiagram
    ...
    OperationRequest ||--o{ WorkItem : splits_into
...
| Operation | Yêu cầu/công việc, Kanban board, checklist, bình luận, file | OperationRequest, WorkItem, Attachment |
| Workflow/Approval | Duyệt/từ chối, lịch sử trạng thái, chuyển bước công việc | WorkflowDefinition, WorkflowInstance, ApprovalTask |
...
sequenceDiagram
    ...
    Manager->>UI: Mở Kanban workflow
    UI->>Ops: Create/Move WorkItem
    Ops->>DB: Cập nhật WorkItem.Status + AuditLog
```
...
flowchart TB
    ...
    Dashboard --> Kanban["/Workflow/Kanban"]
...
| Workflow Kanban | WorkKanbanService + WorkflowController + Kanban Razor board | Đã có code, cần bổ sung ảnh màn hình |
...
| Kanban workflow | Bảng công việc theo cột trạng thái |
...
| TC-11 | Kanban workflow | Tạo thẻ và chuyển trạng thái | Thẻ đổi cột, ghi audit log và giữ tenant scope | Cần chạy |
...
- Khả năng quản lý workflow công việc bằng Kanban theo trạng thái thực tế.
...
Đề tài OmniBizAI hướng đến bài toán thực tế của doanh nghiệp vừa và nhỏ: quản lý vận hành phân tán, thiếu minh bạch và thiếu công cụ hỗ trợ quyết định. Hệ thống đề xuất giải pháp web có quản lý đa cấp, phân quyền, Kanban workflow, dashboard và AI insight. Nếu triển khai đúng blueprint, đề tài có đủ cơ sở để demo một sản phẩm hoàn chỉnh ở mức tốt nghiệp.
...
4. Mở Kanban workflow, tạo thẻ công việc và chuyển trạng thái.
5. Mở dashboard, xem KPI và trạng thái.
6. Hỏi AI: "Những rủi ro vận hành nổi bật tuần này là gì?"
7. Mở audit log để chứng minh hệ thống có truy vết.
8. Xuất báo cáo hoặc hiển thị report filter.
